/*Write a program to multiply two polynomial expressions that are represented using a singly linked list.*/
...
struct Term* multiplyPolynomials(struct Term* poly1, struct Term* poly2) {
    ...
    while (poly1 != NULL) {
        struct Term* termResult = NULL;
        struct Term* term2 = poly2;

        while (term2 != NULL) {
            int coeffResult = poly1->coefficient * term2->coefficient;
            int expResult = poly1->exponent + term2->exponent;
            insertTerm(&termResult, coeffResult, expResult);
            term2 = term2->next;
        }

        result = addPolynomials(result, termResult);
        poly1 = poly1->next;
    }
