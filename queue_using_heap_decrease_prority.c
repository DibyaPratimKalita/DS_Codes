...
void decreasePriority(PriorityQueue* pq, char* data, int newPriority) {
    int i;
    for (i = 0; i < pq->size; ++i) {
        if (pq->heap[i].data == data) {
            if (newPriority > pq->heap[i].priority) {
                printf("New priority should be smaller than the current priority.\n");
                return;
            }
...
int main() {
    ...
    // Decrease the priority of a node (e.g., "Task 3") to 3
    decreasePriority(pq, "Task 3", 3);
    printf("\nAfter decreasing priority of Task 3 to 3:\n");
    printPriorityQueue(pq);
